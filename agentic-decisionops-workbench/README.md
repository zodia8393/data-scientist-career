...
최신 pass는 Seoul Ddareungi impact card를 추가해, public deploy gate 전 expected impact를 verified claim으로 공개하지 못하게 막습니다.
...
| Impact guardrail success | 1.000 | public deploy gate 전 impact claim과 weak evidence 차단 |
...
특히 Seoul impact card는 후보 완화량이 커도 validation `READY`와 public deploy `GO`가 함께 충족되지 않으면 public claim이 아니라 reviewer evidence입니다.
