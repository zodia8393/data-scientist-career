...
| data/tool sources >= 3 | pass | bike-share + NY 511 public traffic events + derived review queue | source refresh script 유지 |
| baseline vs guarded | pass | deterministic workflow + holdout 12/12 | LLM planner ablation 추가 |
| trace/eval report | pass | static report + prepublish audit; Stage 3 Control Tower가 interactive review UI 제공 | trace-to-dashboard deep link는 후속 |
| uncertainty guardrail | pass | heuristic rule 기반 | calibration-based threshold 추가 |
...
| quality floor > 92 | pass | min score 94 | active quality floor ratchet 여부 결정 |
