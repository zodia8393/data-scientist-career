...
OUTPUT_ROOT=/tmp/agentic-decisionops-workbench scripts/run_all.sh
```
...
`run_all`, `pytest`, Saturday validator, Sunday strict validator가 모두 통과하면 public registry packaging 후보로 본다. 단, incident data는 live dispatch authority가 아니므로 reviewer UI와 approval workflow는 Stage 3에서 별도 구현한다.
